...
void read_bfee2(unsigned char *inBytes, mxArray *outCell)
{
	...
	unsigned short bfee_count = inBytes[4] + (inBytes[5] << 8);
        unsigned int sourceAddr = inBytes[6]+(inBytes[7] << 8);
	...
	mxDestroyArray(mxGetField(outCell, 0, "bfee_count"));
        mxDestroyArray(mxGetField(outCell, 0, "source_addr"));
	...
	mxSetField(outCell, 0, "bfee_count", mxCreateDoubleScalar((double)bfee_count));
        mxSetField(outCell, 0, "source_addr", mxCreateDoubleScalar((double)sourceAddr));
